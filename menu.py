...
while place_order:
    ...
    if menu_category.isdigit():
        ...
        if int(menu_category) in menu_items.keys():
            ...
            customer_choice = input("Please select a menu item number: ")

            # 3. Check if the customer typed a number
            if customer_choice.isdigit():
                # Convert the menu selection to an integer
                customer_menu_selection = int(customer_choice)
    ...
    while True:
        ...
        keep_ordering = input("Would you like to keep ordering? (Y)es or (N)o ").title()
...
for each_order in order:
    ...
    item_name = each_order['Item name']
    price = each_order['Price']
    quantity = each_order['Quantity']
    # 8. Calculate the number of spaces for formatted printing
    num_item_spaces = 25 - len(each_order['Item name']) 
...
total_cost = sum([customer_order["Price"] * customer_order["Quantity"] for customer_order in order])
# Multiply the price by quantity for each item in the order list, then sum()
# and print the prices.
print(f"Your total cost for this order comes to: ${total_cost:,.2f}. Have a nice day!")